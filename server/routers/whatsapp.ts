...
import { forceRestartInstance, disconnectInstance, getInstanceStatus, getCurrentQR, getLastError, startPairingInstance, getPairingCode } from "../whatsapp/baileys-manager.js";
...
export const whatsappRouter = router({
  ...
  status: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        pairingCode: getPairingCode(input.botId),
  ...
  requestPairing: protectedProcedure
    .input(z.object({ botId: z.number(), phoneNumber: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const db = getDb();
      const wsId = await getWsId(ctx.user.id);
      ...
      startPairingInstance(input.botId, input.phoneNumber);
      return { ok: true };
    }),
});
