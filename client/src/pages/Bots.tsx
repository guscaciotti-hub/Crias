        ...
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <CardTitle className="text-lg">
            {connected ? "✅ Conectado!"
              ...
              : step === "select" ? "Conectar WhatsApp"
              : step === "phone_input" ? "Código de Pareamento"
              : step === "qr" ? "Escanear QR Code"
            ...
            <div className="space-y-3">
              ...
              {[401, 403, 405, 408, 500].includes(Number(lastError?.code)) && (
                <p className="text-xs text-amber-600">
                  ⚠️ IP do servidor bloqueado pelo WhatsApp. Tente o código de pareamento.
                </p>
              )}
              <div className="flex gap-2">
                <Button variant="outline" className="flex-1" onClick={() => { setStep("select"); setTimedOut(false); }}>Voltar</Button>
                <Button className="flex-1" onClick={() =>
                  step === "qr" ? generateQRMut.mutate({ botId }) : requestPairingMut.mutate({ botId, phoneNumber: phone })
                }>
                  Tentar novamente
                </Button>
              </div>
          ...
          {!connected && !failed && step === "phone_input" && (
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Formato: DDI + DDD + número sem espaços. Ex: <code>5511999998888</code>
              </p>
              <div className="flex gap-2">
            ...
            <div className="text-center">
              {qr ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">O QR atualiza quando expira.</p>
                </>
              ) : isReconnecting ? (
              ...
              {pairingCode ? (
                <>
                  <p className="text-sm text-muted-foreground mb-4">
                    No celular: <strong>WhatsApp → Dispositivos vinculados → Vincular com número de telefone</strong>
                  </p>
                  ...
                  <p className="text-xs text-muted-foreground">Digite este código no WhatsApp. Válido por ~60 segundos.</p>
                </>
              ) : (
                <div className="py-8 flex flex-col items-center gap-3">
                  <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" />
              ...
              <CardContent className="p-5">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <Button size="sm" variant="outline"
                    onClick={() => setQrModal({ botId: bot.id })}>
